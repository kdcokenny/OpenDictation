...
final class RecordingService: NSObject, ObservableObject {
    ...
    private func startLevelMetering() {
        ...
        // Timer.scheduledTimer runs on main run loop, so we're on MainActor
...
// Note: Delegate methods are nonisolated to satisfy protocol requirements.
// AVAudioRecorder calls these on the main thread by default.
...
extension RecordingService: AVAudioRecorderDelegate {
    ...
    nonisolated func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully flag: Bool) {
        if !flag {
            MainActor.assumeIsolated {
                self.logger.warning("Recording finished unsuccessfully")
            }
        }
    }
